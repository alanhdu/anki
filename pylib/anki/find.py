...
import unicodedata
from typing import cast, Any, List, Optional, Set, Tuple, Union, TYPE_CHECKING
...
select distinct(n.id) from cards c, notes n where c.nid=n.id and """
    ...
    def _where(self, tokens: List[str]) -> Tuple[Any, Optional[List[str]]]:
        # state and query
    ...
    def _order(self, order: Union[bool, str]) -> Tuple[str, Any]:
        if not order:
            return "", False
